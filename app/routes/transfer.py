...
def index():
    ...
    branches = Branch.query.filter(Branch.id != branch_id).order_by(Branch.name).all()
    ...
    if request.method == 'POST':
        ...
        cabang_tujuan = request.form.get('cabang_tujuan', '')
        tanggal = request.form.get('tanggal', '')
        keterangan = request.form.get('keterangan', '')

        if not all([divisi, nama_bahan, qty, satuan, cabang_tujuan, tanggal]):
            ...
            db.session.add(txn)
            db.session.commit()
            flash('Transfer Keluar berhasil dicatat!', 'success')
            return redirect(url_for('dashboard.index'))

    return render_template('forms/transfer.html', branches=branches)
